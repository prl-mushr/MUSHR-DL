...
fov_h = 90/57.3 #fov calculation (copied straight from zed's website, so if it fails, sue them not me!)
K_h = m.tan(fov_h/2)
fov_v = 58/57.3
K_v = m.tan(fov_v/2)
...
def xy_to_img(x,y,speed):
    ...
    x,y = XY2img(x,y,cam_height,K_v,K_h,X_max,Y_max,X_Center,Y_Center,pitch,480)
...
for k in range(4):
    data = np.load('MUSHR_320x240_{}.npy'.format(str(k)),allow_pickle=True)
    ...
    np.save('MUSHR_320x240_shuffled_Bezier{}.npy'.format(str(k)),final_out)
    del data
    del inputs
    del output_c
    del output_l
